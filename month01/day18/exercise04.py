...
for val in IterableHelper.get_all_by_condition(list_commodity_infos, lambda item: item.name):
    print(val)
for cid, price in IterableHelper.get_all_by_condition(list_commodity_infos, lambda item: (item.cid, item.price)):
    print(cid, price)

for item in IterableHelper.ascending_order_by(list_commodity_infos, lambda item:len(item.name)):
    print(item.__dict__)
