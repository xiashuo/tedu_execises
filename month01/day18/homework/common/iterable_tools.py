...
class IterableHelper:
    ...
    @staticmethod
    def ascending_order_by(list_target, func):
    ...
    @staticmethod
    def descending_order_by(list_target, func):
        for i in range(len(list_target) - 1):
            for j in range(i + 1, len(list_target)):
                if func(list_target[j]) > func(list_target[i]):
                    list_target[i], list_target[j] = list_target[j], list_target[i]
    ...
    @staticmethod
    def delete_by(list_target, func):
        for i in range(len(list_target) - 1, -1, -1):
            if func(list_target[i]):
                del list_target[i]

    @staticmethod
    def remove_duplicate(list_target,func):
        for i in range(len(list_target)-1,-1,-1):
            for j in range(i):
                if func(list_target[j])==func(list_target[i]):
                    del list_target[i]
